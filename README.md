...
В ветке [FB4S_Config](https://github.com/Sergey1560/Marlin_FB4S/tree/FB4S_Config) находится вариант с минимальными изменениями Marlin (только конфигурация), без поддержки WIFI.
...
Для отправки команд используется tscp socket на порт 8080. Пример с netcat:

```
nc 192.168.0.105 8080
```

Можно отправлять g-код команды, и получать ответ. MKS WIFI модуль возвращает ответ не на все команды.
